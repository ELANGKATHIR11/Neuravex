...
class MultiLayerSegmentationHead(nn.Module):
    ...
    Multi-layer dense segmentation head outputting:
      ...
      - proto_masks: instance mask prototypes (B, num_prototypes, H, W)
    """
    def __init__(self, in_channels: int, num_classes: int = 80, embed_dim: int = 16, num_parts: int = 16, num_prototypes: int = 32):
        super().__init__()
        c = in_channels
        ...
        self.proto_head = nn.Conv2d(c, num_prototypes, 1)
    ...
    def forward(self, q3, q4, q5, out_hw: tuple):
        ...
        inst_emb = F.normalize(inst_raw, dim=1)
        ...
        proto_masks = F.interpolate(self.proto_head(feat), size=out_hw, mode="bilinear", align_corners=False)
